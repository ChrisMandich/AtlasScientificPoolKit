...
#include "secrets.h"                                             //include secrets

WiFiClient  espClient;                                              //declare that this device connects to a Wi-Fi network,create a connection to a specified internet IP address
PubSubClient client(espClient);                                  //declare mqtt pub/sub

char* topic_temp = "AtlasScientific/temp";
char* topic_orp = "AtlasScientific/orp";
char* topic_ph = "AtlasScientific/ph";
...
void setup() {
  ...
  ThingSpeak.begin(espClient);               //enable ThingSpeak connection
      ...
      case READ_RESPONSE:                             //if were in the receiving phase
        if (millis() >= next_step_time) {  //and its time to get the response
          ...
          if (PH.get_error() == Ezo_board::SUCCESS) {                                          //if the PH reading was successful (back in step 1)
            client.publish(topic_ph, String(PH.get_last_received_reading(), 2).c_str(), true);
          ...
          if (ORP.get_error() == Ezo_board::SUCCESS) {                                          //if the ORP reading was successful (back in step 1)
            client.publish(topic_orp, String(ORP.get_last_received_reading(), 2).c_str(), true);
          ...
          if (send_to_thingspeak == true) {                                                    //if we're datalogging
            ...
            if(WiFi.status() == WL_CONNECTED){                                                 //and we're connected to the wifi
              ...
              return_code = ThingSpeak.writeFields(channel_number, thingspeak_api_write);                 //upload the data to thingspeak, read the return code
